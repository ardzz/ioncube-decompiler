...
def mk(nodes, zvals=None, cv=None, thr=None, valid_php=False, debug=False):
...
def test_bottom_tested_while_priming():
    ...
    l = mk(
        ...
        zvals=[
            {"type": 6, "str": b"use_row"},
            {"type": 6, "str": b"next_row"},
            {"type": 1},
        ],
        cv={0: "row"},
    )
...
def test_continue_inside_foreach():
    ...
    l = mk(
        [
            (
                77,
                {"op1": (8, 0), "res": (2, 0), "op2": (0, 7)},
                {"res": 6 * 16},
                0,
            ),  # FE_RESET -> 6
            (
                78,
                {"op1": (2, 0), "op2": (8, 1), "res": UNUSED},
                {"op1": 6 * 16},
                0,
            ),  # FE_FETCH
        ...
        zvals=[{"type": 6, "str": b"tail"}, {"type": 1}],
        cv={0: "items", 1: "v"},
    )
    emit_region(l, 0, l.thr)
    t = out(l)
...
def test_ternary():
    ...
    l = mk(
        ...
        zvals=[{"type": 6, "str": b"yes"}, {"type": 6, "str": b"no"}, {"type": 1}],
        cv={0: "c", 1: "x"},
    )
    emit_region(l, 0, l.thr)
    t = out(l)
...
def test_ternary_multi_node_else_arm():
    ...
    l = mk(
        ...
        zvals=[{"type": 6, "str": b"a"}, {"type": 6, "str": b"k"}, {"type": 1}],
        cv={0: "c", 1: "arr", 2: "x"},
    )
    emit_region(l, 0, l.thr)
...
def test_jmp_set_elvis():
    ...
    l = mk(
        ...
        zvals=[{"type": 6, "str": b"fallback"}, {"type": 1}],
        cv={0: "c", 1: "x"},
    )
    emit_region(l, 0, l.thr)
...
def test_switch_header_degrades_without_chain():
    ...
    l = mk(
        [
            (186, {"op1": (8, 0), "op2": (1, 0), "res": UNUSED}, {}, 0),
            (62, {"op1": (1, 1)}, {}, 0),
        ],
        zvals=[{"type": 7, "str": b"not-a-table"}, {"type": 1}],
        cv={0: "x"},
    )
    emit_node(l, 0, l.thr)
...
def test_switch_jumptable_fallback_without_chain():
    ...
    l = mk(
        ...
        zvals=[
            {"type": 7, "str": b"[1'ai2;4;0;1'bi2;4;0;}0;775;0;1;7;"},
            {"type": 6, "str": b"end"},
            {"type": 1},
        ],
        cv={0: "x"},
...
def test_valid_php_goto_fallback():
    ...
    l = mk(nodes, zvals=[{"type": 6, "str": b"x"}, {"type": 1}], debug=True)
    emit_node(l, 0, l.thr)
...
def test_foreach_key_in_temp_fold():
    ...
    l = mk(
        [
            ...
            (
                78,
                {"op1": (2, 0), "op2": (8, 2), "res": (2, 0)},
                {"op1": 6 * 16, "res": 7 * 16},
                0,
            ),  # FE_FETCH: value CV2, key T7
...
def test_corpus_while_priming_taxgateway():
    """getTaxByUserId: `while ($taxLevelVAT = $iso->fetch())` lifts with the
    ...
    t = r["text"]
    assert t.count("$taxLevelVAT = $iso->fetch();") == 2
    assert "while ($taxLevelVAT) {" in t
...
def test_corpus_switch_seterror_blesta():
    ...
    assert "$error = 'The license is not valid for this version of the system.';" in t
...
def test_corpus_ternary_in_array_literal():
    ...
    r = lift_file(f"{CE}/modules/billing/models/TaxGateway.php")
    t = r["text"]
    assert "'vat' => ($return['vat'] == 1 ? true : false)" in t
    assert "'allcountries' => ($return['countryiso'] == '_ALL' ? true : false)" in t
...
@requires_workspace
def test_corpus_assign_dim_append():
    """BillingGateway getAllRecurringWorkForUserPackage: the dimensionless
    ASSIGN_DIM (op2 = the zeroed unused marker, never a CONST zval) renders
    as the array append — not the fake `$data[0] = ...` first-element write."""
    from ioncube_re.lift import lift_file

    r = lift_file(f"{CE}/modules/billing/models/BillingGateway.php")
    t = r["text"]
    assert "$data[] = $recurringWork;" in t
    assert "$data[0] = $recurringWork;" not in t


@requires_workspace
def test_corpus_fetch_row_named_temp():
    """getAllRecurringWorkForUserPackage: the fetch result is read twice
    (row destructure + loop condition), so it keeps a named temp statement
    instead of a bare $Vn reference — the side-effecting call stays single,
    and the destructure element renders as array-deref on the fetched row."""
    from ioncube_re.lift import lift_file

    r = lift_file(f"{CE}/modules/billing/models/BillingGateway.php")
    t = r["text"]
    assert "$V18 = $result->fetch();" in t
    assert "while ($V18) {" in t
    assert "$id = $V18[0];" in t
    assert "->{'0'}" not in t
    assert "$id = $V7;" not in t
